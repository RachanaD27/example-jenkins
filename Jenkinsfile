				...
				stage('Build and Deploy to Standalone Server'){
					...
					steps { 
						script{
							...
							def targetserverv=deployenv+'_DEPLOY_TARGET_SERVER' def deployusernamev= deployenv+'_DEPLOY_USERNAME' def deploypasswordv=deployenv+'_DEPLOY_PASSWORD' def anypointurlv=deployenv+'_DEPLOY_ANYPOINT_URL'
							
							//Assigning respective variables for deployment def version = env."${versionv}"
							def targetserver=env."${targetserverv}"
							def deployusername=env."${deployusernamev}" def deploypassword=env."${deploypasswordv}" def anypointurl=env."${anypointurlv}"
